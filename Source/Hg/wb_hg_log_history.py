...
#------------------------------------------------------------
#
#   WbHgLogHistoryOptions - option to control which commit logs to show
#
#------------------------------------------------------------
class WbHgLogHistoryOptions(QtWidgets.QDialog):
...
#
#------------------------------------------------------------
class WbHgLogHistoryView(QtWidgets.QWidget):
    uid = 0
    all_windows = {}

    @staticmethod
    def closeAllWindows():
        for window in list( WbHgLogHistoryView.all_windows.values() ):
            window.close()
    ...
    def __init__( self, app, title, icon ):
        ...
        WbHgLogHistoryView.uid += 1
        self.window_uid = WbHgLogHistoryView.uid

        # remember this window to keep the object alive
        WbHgLogHistoryView.all_windows[ self.window_uid ] = self
    ...
    def showCommitLogForRepository( self, hg_project, options ):
        self.log_model.loadCommitLogForRepository( hg_project, options.getLimit(), options.getSince(), options.getUntil() )

    def showCommitLogForFile( self, hg_project, filename, options ):
        self.log_model.loadCommitLogForFile( hg_project, filename, options.getLimit(), options.getSince(), options.getUntil() )
    ...
    def closeEvent( self, event ):
        del WbHgLogHistoryView.all_windows[ self.window_uid ]
...
class WbHgLogHistoryModel(QtCore.QAbstractTableModel):
    ...
    def loadCommitLogForRepository( self, hg_project, limit, since, until ):
        self.beginResetModel()
        self.all_commit_nodes = hg_project.cmdCommitLogForRepository( limit, since, until )
        self.endResetModel()

    def loadCommitLogForFile( self, hg_project, filename, limit, since, until ):
        self.beginResetModel()
...
if __name__ == '__main__':
    ...
    options = WbHgHistoryOptions( None, FakePrefs() )
    if options.exec_():
